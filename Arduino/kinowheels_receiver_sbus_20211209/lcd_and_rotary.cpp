...
#define DELAY_REBOND_TIME 150
#define DELAY_REBOND_TIME_ROTATE 60
...
const int RotaryCLK = 2, RotaryDT = 4;
// const int ButtonPushPin = 6; 
//Statuses for the rotary encoder
int CLKNow, CLKPrevious, DTNow, DTPrevious;
bool buttonState = false; 
extern bool lock_channel_LCD = false;
int counterPush = 0; // in case of double push function
extern uint8_t radio_channel_value = 0; 
...
void initialise_lcd_layout_radio_channel_changing(uint8_t radio_channel_value_for_lcd){
  ...
  display.println("DOUBLE PRESS = PUT TO 0");
...
void rotate(){ 
  ...
  if (CLKNow != CLKPrevious  && (millis() - previous_push) >= DELAY_REBOND_TIME_ROTATE){
...
void pushButton(){
  ...
  if ((_current_push - previous_push) >= DELAY_REBOND_TIME){
